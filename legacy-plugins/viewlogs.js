...
exports.commands = {
	...
	viewlogs2: function (target, room, user, connection, cmd) {
		...
		fs.readFile('logs/chat/' + targetRoom.toLowerCase() + '/' + splitDate[0] + '-' + splitDate[1] + '/' + date + '.txt', 'utf8', (err, data) => {
			...
			data = targetRoom + "|" + date + "|" + JSON.stringify(Legacy.customColors) + "\n" + data;
...
function parseMessage(message, user) {
	...
	case 'c':
		...
		'</small><b><font color="' + Legacy.hashColor(name.substr(1)) + '">' + name.substr(1, name.length) + ':</font></b><em>' +
		Legacy.parseMessage(lineSplit.slice(3).join('|')) + '</em></span>';
		break;
	case 'c:':
		name = lineSplit[3];
		if (name === '~') break;
		...
		message = '<span class="' + div + '"><small>[' + timestamp + ']</small> ' + '<small>' + name.substr(0, 1) +
		'</small>' + Legacy.nameColor(toId(name), true) + '<em>' +
		Legacy.parseMessage(lineSplit.slice(4).join('|')) + '</em></span>';
		break;
	case 'uhtml':
