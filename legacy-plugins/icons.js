...
exports.commands = {
	...
	icon: {
		set: function (target, room, user) {
			...
			this.sendReply("|raw|You have given " + Legacy.nameColor(target[0], true) + " an icon.");
			Rooms('staff').add('|raw|' + Legacy.nameColor((target[0])) + ' has received an icon from ' + Legacy.nameColor(user.name), true).update();
			this.privateModCommand("|raw|(" + target[0] + " has recieved icon: <img src='" + target[1] + "' width='32' height='32'> from " + user.name + ".)");
			if (Users(target[0]) && Users(target[0]).connected) Users(target[0]).popup("|html|" + Legacy.nameColor(user.name, true) + " has set your userlist icon to: <img src='" + target[1] + "' width='32' height='32'><br><center>Refresh, If you don't see it.</center>");
		...
		delete: function (target, room, user) {
			...
			Rooms('staff').add('|raw|' + Legacy.nameColor(user.name, true) + " removed " + target + "'s icon.").update();
			this.privateModCommand("(" + target + "'s icon was removed by " + user.name + ".)");
			if (Users(target) && Users(target).connected) Users(target).popup("|html|" + Legacy.nameColor(user.name, true) + " has removed your userlist icon.");
		},
	},
	iconhelp: [
		"Commands Include:",
