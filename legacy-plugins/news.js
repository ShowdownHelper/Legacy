...
function generateNews() {
	...
	for (let i = 0; i < keys.length; i++) {
		...
		newsDisplay.push(`<h4>${keys[i]}</h4>${newsData[1]}<br /><br />—${Legacy.nameColor(newsData[0], true)} <small>on ${newsData[2]}</small>`);
	}
	return newsDisplay;
}
...
Legacy.showNews = function (userid, user) {
	...
	if (newsDisplay.length > 0) {
		...
		return user.send(`|pm| Legacy Server|${user.getIdentity()}|/raw ${newsDisplay}`);
	}
};
...
exports.commands = {
	...
	serverannouncements: {
		...
		view: function (target, room, user) {
			...
			let output = `<center><strong>Legacy News:</strong></center>${generateNews().join(`<hr>`)}${showSubButton(user.userid)}`;
		...
		subscribe: function (target, room, user) {
			...
			if (Db.NewsSubscribers.has(user.userid)) return this.errorReply("You are alreading subscribing Legacy News.");
			Db.NewsSubscribers.set(user.userid, true);
			this.sendReply("You have subscribed Legacy News.");
			this.popupReply("|wide||html|You will receive Legacy News automatically once you connect to the Legacy next time.<br><hr><center><button class='button' name='send' value ='/news'>View News</button></center>");
		},
		unsubscribe: function (target, room, user) {
			if (!user.named) return this.errorReply('You must choose a name before unsubscribing');
			if (!Db.NewsSubscribers.has(user.userid)) return this.errorReply("You have not subscribed Legacy News.");
			Db.NewsSubscribers.remove(user.userid);
			this.sendReply("You have unsubscribed Legacy News.");
			this.popupReply("|wide||html|You will no longer automatically receive Legacy News.<br><hr><center><button class='button' name='send' value='/news'>View News</button></center>");
		},
	},
	serverannouncementshelp: ["/news view - Views current Legacy news.",
		"/news delete [news title] - Deletes announcement with the [title]. Requires @, &, ~",
		"/news add [news title], [news desc] - Adds news [news]. Requires @, &, ~",
		"/news subscribe - Subscribes to Legacy News.",
		"/news unsubscribe - Unsubscribes to Legacy News.",
	],
};
