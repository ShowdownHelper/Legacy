...
exports.commands = {
	...
	checkroomrequest: function (target, room, user) {
		...
		let output = `<center><h1>Legacy Room Request</h1></center><b>Requester</b>: ${target} <br/><b>Room Name</b>: ${curRequest.name}<br/><b>Room Type</b>: ${curRequest.type}<br/><b>Description</b>: ${curRequest.desc}<br/>`;
		if (user.can('roomowner')) output += `${(curRequest.staff ? `The requester is a Legacy global staff member` : (curRequest.trusted ? `The requester is a trusted user.` : ``))}`;
