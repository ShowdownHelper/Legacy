...
exports.commands = {
	...
	credits: function (target, room, user) {
		let popup = "|html|" + "<font size=5 color=#0066ff><u><b>SpacialGaze Credits</b></u></font><br />" +
			...
			"- " + Legacy.nameColor('Legacy Saffron', true) + " (Owner, Sysadmin, Development, all that GOOD shit)<br />"
			"<br />" +
			"<u><b>Major Contributors:</b></u><br />" +
			"- " + Legacy.nameColor('Finny', true) + " (Idk I'm the one writing this shit, surely that's worth something)<br />" +
			"- " + Legacy.nameColor('Showdown Helper', true) + " (Host, being sexy)<br />"
		user.popup(popup);
	},
	...
	regdate: function (target, room, user, connection) {
		...
		function regdateReply(date) {
			if (date === 0) {
				return Legacy.nameColor(target, true) + " <b><font color='red'>is not registered.</font></b>";
			} else {
				let d = new Date(date);
				...
				return Legacy.nameColor(target, true) + " was registered on <b>" + DayNames[d.getUTCDay()] + ", " + MonthNames[d.getUTCMonth()] + ' ' + d.getUTCDate() + ", " + d.getUTCFullYear() + "</b> at <b>" + d.getUTCHours() + ":" + d.getUTCMinutes() + ":" + d.getUTCSeconds() + " UTC.</b>";
			}
			//room.update();
		}
	},
	...
	seen: function (target, room, user) {
		...
		if (targetUser && targetUser.connected) return this.sendReplyBox(Legacy.nameColor(targetUser.name, true) + " is <b><font color='limegreen'>Currently Online</b></font>.");
		target = Chat.escapeHTML(target);
		let seen = Db.seen.get(toId(target));
		if (!seen) return this.sendReplyBox(Legacy.nameColor(target, true) + " has <b><font color='red'>never been online</font></b> on this server.");
		this.sendReplyBox(Legacy.nameColor(target, true) + " was last seen <b>" + Chat.toDurationString(Date.now() - seen, {precision: true}) + "</b> ago.");
	},
	...
	usetoken: function (target, room, user, connection, cmd, message) {
		...
		switch (target[0]) {
		case 'avatar':
			msg = '/html <center>' + Legacy.nameColor(user.name, true) + ' has redeemed a avatar token.<br/><img src="' + target[1] + '" alt="avatar"/><br/>';
			...
			return Legacy.messageSeniorStaff(msg);
		case 'declare':
			msg += '/html <center>' + Legacy.nameColor(user.name, true) + ' has redeemed a global declare token.<br/> Message: ' + target[1] + "<br/>";
			msg += '<button class="button" name="send" value="/globaldeclare ' + target[1] + '">Globally Declare the Message</button></center>';
			delete user.tokens[target[0]];
			return Legacy.messageSeniorStaff(msg);
		case 'color':
			msg += '/html <center>' + Legacy.nameColor(user.name, true) + ' has redeemed a custom color token.<br/> hex color: <span' + target[1] + '<br/>';
		...
		case 'title':
			...
			msg += '/html <center>' + Legacy.nameColor(user.name, true) + ' has redeem a title token.<br/> title name: ' + target[1] + '<br/>';
		...
		case 'emote':
			...
			msg += '/html <center>' + Legacy.nameColor(user.name, true) + ' has redeem a emote token.<br/><img src="' + target[2] + '" alt="' + target[1] + '"/><br/>';
		...
		case 'disableintroscroll':
			...
			msg += '/html <center>' + Legacy.nameColor(user.name, true) + ' has redeemed roomintro scroll disabler token.<br/>';
	...
	dailybonus: function (target, room, user) {
		...
		if ((86400000 - nextBonus) <= 0) return Legacy.giveDailyReward(user.userid, user);
