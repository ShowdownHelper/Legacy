...
class Voucher {
	constructor(userid, voucher, item, expires, id) {
		...
		this.id = (id ? id : Legacy.vouchers.storageForVocuherIds);
		if (!id) Legacy.vouchers.storageForVocuherIds++;
	}
	...
	redeem() {
		...
		case 'item':
			...
			case 'customavatar':
				...
				//Legacy.messageSeniorStaff(Users(this.userid).name + " has purchased a Custom Avatar. Please contact this user to setup their Custom Avatar.");
				break;
			case 'customcolor':
				...
				//Legacy.messageSeniorStaff(Users(this.userid).name + " has purchased a Custom Color. Please contact this user to setup their Custom Color.");
				break;
			case 'customtitle':
				...
				//Legacy.messageSeniorStaff(Users(this.userid).name + " has purchased a Custom Title. Please contact this user to setup their Custom Color.");
				break;
			case 'customicon':
				...
				//Legacy.messageSeniorStaff(Users(this.userid).name + " has purchased a Userlist Icon. Please contact this user to setup their Userlist Icon.");
				break;
			}
			break;
		case 'boostuno':
...
function writeFile() {
	...
	fs.writeFile('config/vouchers.json', JSON.stringify(Legacy.vouchers));
}
	...
	if (!noRead) {
		...
		for (let key in raw) {
			if (key === 'storageForVocuherIds') {
				Legacy.vouchers.storageForVocuherIds = raw[key];
				continue;
			}
			Legacy.vouchers[key] = [];
			for (let i = 0; i < raw[key].length; i++) {
				...
				Legacy.vouchers[key].push(reVouch);
			}
		}
		if (!Legacy.vouchers.storageForVocuherIds) Legacy.vouchers.storageForVocuherIds = 1;
	} else {
		Legacy.vouchers = {
			storageForVocuherIds: 1,
		};
...
exports.commands = {
	...
	voucher: {
		give: function (target, room, user, connection, cmd, message) {
			if (Legacy.storageForVocuherIds === -1) return this.errorReply('An error occured while loading vouchers. They cannot be used at this time.');
			...
			if (!Legacy.vouchers[targetUser.userid]) Legacy.vouchers[targetUser.userid] = [];
			...
			if (index === 0 || index === 1) {
				...
				if (index === 0) {
					...
					Legacy.vouchers[targetUser.userid].push(voucher);
		...
		take: function (target, room, user, connection, cmd, message) {
			...
			let targetUser = Users(toId(target[0]));
			if (!targetUser) return this.errorReply('User ' + target[0] + ' not found.');
			if (!Legacy.vouchers[targetUser.userid] || Legacy.vouchers[targetUser.userid].length === 0) return this.errorReply(targetUser.name + ' has no vouchers.');
			if (isNaN(Number(target[1]))) return this.errorReply('Ids must be a number.');
			for (let key in Legacy.vouchers[targetUser.userid]) {
				if (Legacy.vouchers[targetUser.userid][key].id === Number(target[1])) {
					delete Legacy.vouchers[targetUser.userid][key];
					Legacy.vouchers[targetUser.userid].splice(key, 1);
					writeFile();
		...
		list: function (target, room, user, connection, cmd, message) {
			...
			if (!Legacy.vouchers[targetUser.userid] || Legacy.vouchers[targetUser.userid].length === 0) return this.errorReply(targetUser.name + ' has no vouchers.');
			let list = Legacy.vouchers[targetUser.userid];
		...
		redeem: function (target, room, user, connection, cmd, message) {
			...
			if (!Legacy.vouchers[user.userid] || Legacy.vouchers[user.userid].length === 0) return this.errorReply('You have no vouchers.');
			let index = -1;
			for (let i = 0; i < Legacy.vouchers[user.userid].length; i++) {
				if (Legacy.vouchers[user.userid][i].id === target) {
					index = i;
					break;
				}
			}
			if (index === -1) return this.errorReply('You do not own voucher id ' + target + '.');
			switch (Legacy.vouchers[user.userid][index].redeem()) {
			case true:
				//Successfuly redeemed
				delete Legacy.vouchers[user.userid][index];
				Legacy.vouchers[user.userid].splice(index, 1);
				writeFile();
				return true;
				//break;
			case false:
			...
			case 'expired':
				...
				//break;
			case 'active':
				return this.sendReply('You already have an active ' + toId(Legacy.vouchers[user.userid][index].goodFor) + '.');
				//break;
			case 'token':
				return this.sendReply('You already have a token for a ' + toId(Legacy.vouchers[user.userid][index].item) + '.');
				//break;
			default:
