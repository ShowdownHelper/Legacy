...
global.currencyName = 'Gold';
global.currencyPlural = 'Gold';
...
function rankLadder(title, type, array, prop, group) {
	...
	for (let i = 0; i < array.length; i++) {
		if (i === 0) {
			...
			tableRows += '<tr><td ' + first + '>' + (i + 1) + midColumn + Legacy.nameColor(array[i].name, true) + midColumn + array[i][prop] + '</td></tr>';
		} else if (i === 1) {
			midColumn = '</td><td ' + second + '>';
			tableRows += '<tr><td ' + second + '>' + (i + 1) + midColumn + Legacy.nameColor(array[i].name, true) + midColumn + array[i][prop] + '</td></tr>';
		} else if (i === 2) {
			midColumn = '</td><td ' + third + '>';
			tableRows += '<tr><td ' + third + '>' + (i + 1) + midColumn + Legacy.nameColor(array[i].name, true) + midColumn + array[i][prop] + '</td></tr>';
		} else {
			midColumn = '</td><td ' + tdStyle + '>';
			tableRows += '<tr><td ' + tdStyle + '>' + (i + 1) + midColumn + Legacy.nameColor(array[i].name, true) + midColumn + array[i][prop] + '</td></tr>';
		}
	}
...
exports.commands = {
	...
	wallet: function (target, room, user) {
		...
		Economy.readMoney(userid, money => {
			this.sendReplyBox(Legacy.nameColor(target, true) + " has " + money + ((money === 1) ? " " + currencyName + "." : " " + currencyPlural + "."));
			//if (this.broadcasting) room.update();
		});
	},

	gg: 'givecurrency', //You can change "gg" and "givegold" to your currency name for an alias that applies to your currency Example: AwesomeBucks could be "ga" and "giveawesomebucks"
	givegold: 'givecurrency',
	gc:'givecurrency',
		...
		Economy.writeMoney(targetUser, amount, () => {
			Economy.readMoney(targetUser, newAmount => {
				if (Users(targetUser) && Users(targetUser).connected) {
					...
					' from ' + Legacy.nameColor(user.userid, true) + '.');
				}
	...
	tg: 'takecurrency', //You can change "tg" and "takegold" to your currency name for an alias that applies to your currency Example: AwesomeBucks could be "ta" and "takeawesomebucks"
	takegold: 'takecurrency',
	tc:'takecurrency',
	takecurrency: function (target, room, user, connection, cmd) {
		...
		let targetUser = splitTarget[0];
		if (toId(targetUser) === user.userid) return this.errorReply("You cannot transfer gold to yourself.");
		if (toId(targetUser).length < 1) return this.sendReply("/" + cmd + " - [user] may not be blank.");
		if (toId(targetUser).length > 19) return this.sendReply("/" + cmd + " - [user] can't be longer than 19 characters");
		...
		Economy.writeMoney(targetUser, -amount, () => {
			Economy.readMoney(targetUser, newAmount => {
				if (Users(targetUser) && Users(targetUser).connected) {
					Users.get(targetUser).popup('|html|' + Legacy.nameColor(user.userid, true) + ' has removed ' + amount + ' ' + (amount === 1 ? currencyName : currencyPlural) +
					' from you.<br />');
				}
	...
	confirmtransfergold: 'transfercurrency', //You can change "transfergold" and "confirmtransfergold" to your currency name for an alias that applies to your currency Example: AwesomeBucks could be "transferawesomebucks" and "confirmtransferawesomebucks"
	transfergold: 'transfercurrency',
	...
	transfercurrency: function (target, room, user, connection, cmd) {
		...
		Economy.readMoney(user.userid, money => {
			...
			if (cmd !== 'confirmtransfercurrency' && cmd !== 'confirmtransfergold') {
				return this.popupReply('|html|<center>' +
					...
					'Confirm transfer to <br><b style = "color:' + Legacy.hashColor(targetUser) + '; text-shadow: 1px 1px 1px rgba(0, 0, 0, 0.8)">' + Chat.escapeHTML(targetUser) + '</b></button></center>'
				);
			}
					...
					Economy.readMoney(targetUser, firstAmount => {
						Economy.readMoney(user.userid, secondAmount => {
							...
							if (Users.getExact(targetUser) && Users.getExact(targetUser).connected) {
								Users.getExact(targetUser).send('|popup||html|' + Legacy.nameColor(user.name, true) + " has sent you " + amount + ((amount === 1) ? " " + currencyPlural : " " + currencyPlural));
							}
						});
					});
				});
