...
Legacy.ignoreEmotes = {};
try {
	Legacy.ignoreEmotes = JSON.parse(fs.readFileSync('config/ignoreemotes.json', 'utf8'));
} catch (e) {}
...
function parseEmoticons(message) {
	if (emoteRegex.test(message)) {
		message = Legacy.parseMessage(message).replace(emoteRegex, function (match) {
...
Legacy.parseEmoticons = parseEmoticons;
...
exports.commands = {
	...
	emoticon: function (target, room, user) {
		...
		case 'add':
			...
			Rooms('upperstaff').add('|raw|' + Legacy.nameColor(user.name, true) + ' has added the emote "' + Chat.escapeHTML(parts[1]) +
				'": <img width="40" height="40" src="' + parts[2] + '">').update();
			Legacy.messageSeniorStaff('/html ' + Legacy.nameColor(user.name, true) + ' has added the emote "' + Chat.escapeHTML(parts[1]) +
		...
		case 'ignore':
			if (Legacy.ignoreEmotes[user.userid]) return this.errorReply("You are already ignoring emoticons.");
			Legacy.ignoreEmotes[user.userid] = true;
			fs.writeFileSync('config/ignoreemotes.json', JSON.stringify(Legacy.ignoreEmotes));
			this.sendReply("You are now ignoring emoticons.");
			break;

		case 'unignore':
			if (!Legacy.ignoreEmotes[user.userid]) return this.errorReply("You aren't ignoring emoticons.");
			delete Legacy.ignoreEmotes[user.userid];
			fs.writeFileSync('config/ignoreemotes.json', JSON.stringify(Legacy.ignoreEmotes));
